...
5. **Decision gate:**
   ...
   - ANY below 8.5? → Revise the draft NOW addressing ALL feedback. Stay in `CRITIQUE` for next cycle.
   - Round 3 and still below 8.5? → Park it. Set `phase: "PARKED"`. Start fresh next cycle.

6. Commit: `git add drafts/ && git commit -m "Critique round {N}: {headline} ({avg score})" && git push origin main`

**EXIT → Phase 4 (if all 8.5+) or repeat Phase 3 next cycle.**
...
```
   - If PUBLISHED_TODAY >= 1: Stay in SHIP phase. Commit nothing. Exit. Try next cycle.
...
2. If ANY check fails: fix and push. Stay in QA for next cycle.
...
**EXIT → Next cycle starts Phase 1 with a new article.**
