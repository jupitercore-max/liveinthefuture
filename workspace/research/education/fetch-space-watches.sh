#!/bin/bash
# Fetch all 2195 space watch records from Looker Studio API
# 88 pages × 25 rows each, fired in parallel
...
fetch_page() {
  ...
  curl -s 'https://lookerstudio.google.com/u/0/batchedDataV2?appVersion=20260330_0701' \
    ...
    -H 'x-client-data: CKmdygEIk6HLAQiGoM0BCNi7zwEIlrzPAQiIvs8BCLG+zwEIkL/PAQjGv88BGLGKzwEY1b3PAQ==' \
    -H "x-rap-xsrf-token: $XSRF" \
    --data-raw "$body" \
    -o "$OUTDIR/page_${start}.json" \
    -w "%{http_code}" 2>/dev/null
}

echo "Starting parallel fetch of 88 pages..."
PIDS=()
for start in $(seq 0 25 2175); do
  fetch_page "$start" &
  PIDS+=($!)
...
echo "Files with data: $TOTAL, Empty: $EMPTY"
ls -la "$OUTDIR/" | head -5
wc -c "$OUTDIR"/page_0.json "$OUTDIR"/page_25.json "$OUTDIR"/page_2175.json 2>/dev/null
